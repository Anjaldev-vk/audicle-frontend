...
export default function AcceptInvitePage() {
  ...
  if (error) {
    return (
      <div className="font-[Inter,sans-serif] min-h-screen text-text-muted flex flex-col items-center justify-center p-6 bg-brand-bg transition-colors duration-300">
        <div className="w-full max-w-md p-12 rounded-3xl border border-red-500/10 shadow-2xl text-center bg-brand-surface">
          ...
          <h2 className="text-2xl font-bold text-text-main mb-4">Invitation Error</h2>
          <p className="text-text-muted text-sm mb-8">{error}</p>
          <Link to="/login" className="inline-block px-8 py-4 bg-brand-highlight hover:opacity-80 text-text-main border border-brand-border rounded-2xl text-xs font-bold tracking-widest transition-all">
            GO TO LOGIN
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="font-[Inter,sans-serif] min-h-screen text-gray-400 flex flex-col items-center justify-center p-6" style={{ background: '#050505' }}>
      <div 
        className="w-full max-w-lg p-12 rounded-3xl border border-brand-border shadow-2xl transition-all text-center bg-brand-surface"
      >
        ...
        <h2 className="text-3xl font-bold text-text-main tracking-tight mb-4">Join {inviteData.organisation}</h2>
        ...
        <div className="flex flex-col gap-4">
          ...
          <Link 
            to="/login"
            className="w-full px-5 py-4 bg-brand-highlight hover:opacity-80 text-text-main rounded-2xl text-xs font-bold tracking-widest transition-all border border-brand-border no-underline"
          >
            ALREADY HAVE AN ACCOUNT? LOG IN
          </Link>
        </div>

        <p className="mt-8 text-[0.65rem] text-text-muted uppercase tracking-widest">
