  ...
  return (
    <form onSubmit={handleSubmit} className="relative group w-full max-w-2xl mx-auto">
      ...
      <div className="relative flex items-center bg-brand-surface border border-brand-border rounded-2xl px-6 py-4 transition-all group-focus-within:border-blue-500/50 group-focus-within:shadow-2xl group-focus-within:shadow-blue-600/10">
        {isLoading ? <Loader2 className="w-5 h-5 text-blue-500 animate-spin mr-4" /> : <Search className="w-5 h-5 text-text-muted mr-4 group-focus-within:text-blue-400 transition-colors" />}
        <input
          type="text"
          placeholder="Search through all your meeting intelligence..."
          className="bg-transparent border-none focus:ring-0 text-text-main placeholder:text-text-muted/50 w-full outline-none text-base"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
        />
        <div className="hidden sm:flex items-center gap-1 px-2 py-1 bg-brand-bg/50 border border-brand-border rounded-lg">
          <span className="text-[10px] font-bold text-text-muted">CMD</span>
